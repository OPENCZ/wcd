...
- `xs`：当屏幕像素`>=0px`时生效，其值可接受`1` ~ `24`；
- `sm`：当屏幕像素`>=768px`时生效，其值可接受`1` ~ `24`，反之隐藏；
- `md`：当屏幕像素`>=992px`时生效，其值可接受`1` ~ `24`，反之隐藏；
- `lg`：当屏幕像素`>=1200px`时生效，其值可接受`1` ~ `24`，反之隐藏；
- `xl`：当屏幕像素`>=1920px`时生效，其值可接受`1` ~ `24`，反之隐藏。
