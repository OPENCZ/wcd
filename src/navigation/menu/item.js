...
class MenuItem extends CreateHTMLElement {
    ...
    connectedCallback() {
        function f(node) {
            let result = [];
            while (node) {
                node.tagName?.toLocaleLowerCase() === `${config.prefix}-submenu` ? result.push(node) : '';

                if (node.parentNode?.tagName?.toLocaleLowerCase() === `${config.prefix}-menu`) break;
