 ...
 * @returns {*}		转义后的json
 */
export function JSONStringFormat(str) {
...
class $$ {
	...
	parents(selector, ...restOfSelector) {
		...
		this.each(node => {
			while (node) {
				if (
					node.nodeType >= 9 ||
					node.parentNode?.tagName?.toLocaleLowerCase() === 'body'
				)
					break;

				for (let i = 0; i < args.length; i++) {
					if (
						node.classList?.contains(args[i].replace(/^./, '')) ||
						`#${node.id}` === args[i] ||
						node.tagName.toLocaleLowerCase() === args[i]
					) {
						result.push(node);
						break;
					}
	...
	getStyle(styleName) {
		let result = this.each(item => {
			...
			return styleValue.indexOf('px') > -1
	...
	bindEvents(options) {
		$(options.el)
			.on('touchstart', ev => {
				...
				let startResultValue = options?.onTouchStart?.call(
					ev.currentTarget,
					ev,
					this.data,
				);
			...
			.on('touchend', ev => {
				...
				let endResultValue = options?.onTouchEnd?.call(
					ev.currentTarget,
					ev,
					this.data,
				);
...
export const loadingIcon = {
	...
	`,
	html: `
		<svg class="wc-icon-loading" viewBox="0 0 1024 1024" xmlns="http://www.w3.org/2000/svg">
