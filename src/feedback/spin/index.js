...
class SpinComponent extends CreateHTMLElement {
	...
	show() {
		let el = $(this.shadowRoot)
			.find(`.loading-${this.type}-wrapper`)
			.addClass('loading');
	...
	setMaskStyle() {
		let top = $(this).attr('mask-icon-top'),
			el = $(this.shadowRoot)
				...
				.css({
					backdropFilter: `blur(${pxToVw(
						this.getAttribute('mask-blur') || 6,
					)})`,
	...
	render() {
		return `
            <style>
                ...
                :host([loading]) .loading-more-wrapper.loading, 
                :host([loading]) .loading-mask-wrapper.loading {
                    display: flex;
                }
                
                :host([loading=true]) .loading-more-wrapper.show, 
                :host([loading=true]) .loading-mask-wrapper.show {
                    opacity: 1;
                }
                
                :host([loading=false]) .loading-more-wrapper.hide, 
                :host([loading=false]) .loading-mask-wrapper.hide {
                    opacity: 0;
                }
