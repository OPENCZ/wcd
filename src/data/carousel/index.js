import {$, CreateHTMLElement, customElementsDefine, Touch, config} from '../../utils';
import './item';
...
class CarouselComponent extends CreateHTMLElement {
    ...
    loopTouchStart() {
        ...
        if (!this.activeIndex) {
            $(this.carouselItemsElement).eq(-1).css(this.leftOrTop, this.beforeFirstElementPosition);
        }

        if (this.activeIndex === this.carouselItemsElement.length - 1) {
            $(this.carouselItemsElement).eq(0).css(this.leftOrTop, this.afterLastElementPosition);
        }
    }
    ...
    loopTouchMove(moveDistance, pageValue) {
        ...
        if (!this.activeIndex && isToRightOrBottom && this.carouselItemsElement.length > 3) {
            $(this.carouselItemsElement).eq(-2).css(this.leftOrTop, this.beforeFirstElementPosition);
        }
        ...
        if (this.activeIndex === this.carouselItemsElement.length - 1 && isToLeftOrTop) {
            $(this.carouselItemsElement).eq(1).css(this.leftOrTop, this.afterLastElementPosition);
        }
    }
    ...
    loopTouchEnd(touchend, newActiveIndex) {
        ...
        this.timer = setTimeout(() => {
            ...
            this.activeIndex == -1 ? (this.activeIndex = this.carouselItemsElement.length - 1) : '';

            this.activeIndex == this.carouselItemsElement.length ? (this.activeIndex = 0) : '';
            ...
            $(this.carouselSliderElement)
                ...
                .css('transform', `translate3d(${touchend.pageX}px, ${touchend.pageY}px, 0)`, true);
    ...
    getPosition() {
        let lastElementOffset = $(this.carouselItemsElement).eq(-1).offset(),
            afterLastElementPosition =
                lastElementOffset[this.widthOrHeight] + lastElementOffset[this.leftOrTop] + this.marginValue,
    ...
    getActiveElementPosition(activeIndex) {
        ...
        if ($(this).offset()[this.widthOrHeight] - activeElementOffset[this.widthOrHeight] === 1) {
            activeElementOffset[this.widthOrHeight] = $(this).offset()[this.widthOrHeight];
        }

        return (
            -activeElementOffset[this.leftOrTop] +
            ($(this).offset()[this.widthOrHeight] - activeElementOffset[this.widthOrHeight]) / 2
        );
    }
