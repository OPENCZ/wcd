...
class CheckboxComponent extends CreateHTMLElement {
    ...
    onClick() {
        $(this.shadowRoot).on('click', ev => {
            ...
            $(`${config.prefix}-checkbox[data-trigger-event=true]`).removeAttr('data-trigger-event');
    ...
    CustomEventResultParams() {
        ...
        if (result.name) {
            ...
            let // 所有同名组件元素
                ...
                // 所有同名选中且禁用状态组件元素
                allCheckedAndDisabledCheckbox = $(`${selector}[checked=true][disabled=true]`);

            result.isCheckedAll =
                allCheckbox.length ===
                allCheckedCheckbox.length + allDisabledCheckbox.length - allCheckedAndDisabledCheckbox.length;
