...
class InputComponent extends CreateHTMLElement {
	...
	static get observedAttributes() {
		return ['type', 'value', 'disabled', 'placeholder', 'show-clear'];
	}
	...
	set value(value) {
		...
		value || $(this).attr('show-clear') === 'true'
		...
		if (
			...
			$(this.shadowRoot)
				...
				.on('click', ev => {
					...
					this.type = $(ev.currentTarget).hasClass('change')
						? 'text'
						: 'password';
				});
		}
	...
	render() {
		...
		return `
            <style>
                ...
                :host([disabled=true]) .input-wrapper.focus, 
                :host([disabled=true]) .input-wrapper:hover {
                ...
                :host([border=none]) .input-wrapper, 
                :host([border=top]) .input-wrapper, 
                :host([border=right]) .input-wrapper, 
                :host([border=bottom]) .input-wrapper, 
                :host([border=left]) .input-wrapper {
                    border: none;
                }
					...
					$(this).attr('clear') === 'true'
						? `
							<span class="icon-box clear">
								<svg viewBox="0 0 1024 1024" xmlns="http://www.w3.org/2000/svg" fill="var(--color-border)">
